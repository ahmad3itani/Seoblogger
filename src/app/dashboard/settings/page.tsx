...
export default function SettingsPage() {
    ...
    const handleSaveCredentials = async () => {
        ...
        try {
            ...
            if (res.ok) {
                setCredentialsSaved(true);
                setSaveSuccess("Credentials saved successfully! You can now connect your Blogger account.");
                setTimeout(() => setSaveSuccess(""), 5000);
            } else {
                const data = await res.json();
                ...
                {hasConnection && blogs.length > 0 && (
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            <p className="text-sm text-muted-foreground">Your connected blogs:</p>
                            <Button
                                variant="ghost"
                                size="sm"
                        ...
                        {blogs.map((blog) => (
                            <div key={blog.id} className={`flex items-center gap-3 p-4 rounded-xl border transition-all ${blog.isDefault ? "bg-[#6C4CF1]/5 border-[#6C4CF1]/30 shadow-sm" : "bg-card border-border/50 hover:border-[#6C4CF1]/20"}`}>
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${blog.isDefault ? "bg-[#6C4CF1] text-white shadow-md shadow-[#6C4CF1]/20" : "bg-muted text-muted-foreground"}`}>
                                    {blog.isDefault ? (
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-semibold truncate text-foreground flex items-center gap-2">
                                        {blog.name}
                                        {blog.isDefault && (
                                            <Badge variant="secondary" className="px-2 py-0 h-5 text-[10px] bg-[#6C4CF1] text-white border-0">
                                                Default
                                            </Badge>
                                        )}
                                    </p>
                                    <a href={blog.url} target="_blank" rel="noreferrer" className="text-xs text-muted-foreground truncate hover:text-[#6C4CF1] hover:underline transition-colors mt-0.5 block">
                                        {blog.url}
                                    </a>
                                </div>
                                {!blog.isDefault && (
                                    <Button
                                        variant="ghost"
                                        size="sm"
                                        className="h-8 text-xs hover:bg-orange-500/10 hover:text-[#6C4CF1]"
